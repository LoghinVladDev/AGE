...
concept SameAs =
  cds::meta::IsSame<L, R>::value;

template <typename T>
using Dereferenceable = decltype(*cds::meta::referenceOf<T>());
} // namespace detail

template <typename T>
concept Dereferenceable = requires(T i) {
  typename detail::Dereferenceable<T>;
};

template <typename L, typename R>
concept SameAs =
    detail::SameAs<L, R> && detail::SameAs<R, L>;

template <typename L, typename R>
concept ConvertibleTo =
    cds::meta::IsConvertible<L, R>::value;
...
concept SentinelFor =
  cds::meta::EqualToPossible<S, I>::value;
...
concept TotallyOrdered =
  cds::meta::EqualToPossible<T, T>::value &&
  PartiallyOrderedWith<T, T>;

template <typename T>
concept RandomAccessIterator =
  BidirectionalIterator<T> &&
  TotallyOrdered<T> &&
  requires(T i, T const j, decltype(i - j) const n) {
    { i += n } -> SameAs<T&>;
    { j + n } -> SameAs<T>;
    { n + j } -> SameAs<T>;
    { i -= n } -> SameAs<T&>;
